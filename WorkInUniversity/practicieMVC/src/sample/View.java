...
public class View extends Group {
        ...
        private void createPane(){
            ...
            Text personStatus = new Text(model.getStatus().get());
            personStatus.setFont(Font.font("Tahoma", FontWeight.NORMAL, 15));
            personStatus.textProperty().bind(model.getStatus());
            grid.add(personStatus, 1, 3);

            Text personTitle$ = new Text("$");
            personTitle$.setFont(Font.font("Tahoma", FontWeight.NORMAL, 15));
            grid.add(personTitle$, 0, 4);


            Text person$ = new Text();
            person$.setFont(Font.font("Tahoma", FontWeight.NORMAL, 15));
            person$.textProperty().bind(model.person$IntegerProperty().asString());
            grid.add(person$, 1, 4);

            Text personStatusTitle$ = new Text("Status $");
            personStatusTitle$.setFont(Font.font("Tahoma", FontWeight.NORMAL, 15));
            grid.add(personStatusTitle$, 0, 5);


            Text personStatus$ = new Text(model.getStatus$().get());
            personStatus$.setFont(Font.font("Tahoma", FontWeight.NORMAL, 15));
            personStatus$.textProperty().bind(model.getStatus$());
            grid.add(personStatus$, 1, 5);
